...
class BasedSession(requests.Session):
    """A session that can solve BasedFlare challenges."""
    ...
    def request(self, method, url, **kwargs):
        ...
        if not self.cookies.get('_basedflare_pow', domain=f'.{domain}') and not url.endswith('.basedflare/bot-check'):
    ...
    def __solve_challenge(self, domain):
        ...
        solution = self.solvers[algorithm](*challenge['ch'].split('#')[0:2], *map(int, params.split('#')))
    ...
    def __get_challenge(self, domain):
        ...
        if res.status_code != requests.codes.forbidden:
            raise ChallengeRequestError(f'Unexpected status code {res.status_code} when fetching the challenge')
